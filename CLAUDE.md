...
# 특정 앱만 실행할 때
yarn workspace @apps/fit-track dev
yarn workspace @apps/fit-track build
yarn workspace @apps/fit-track lint
```
...
- 새 의존성 추가 시 **워크스페이스 루트**가 아닌 **해당 앱/패키지 디렉토리**에서 추가: `yarn workspace @apps/fit-track add [패키지]`
- `packages/ui`의 shadcn/ui 컴포넌트 추가 시 `packages/ui` 기준으로 `npx shadcn@latest add` 실행 (`.claude/components.json` 위치 확인)
